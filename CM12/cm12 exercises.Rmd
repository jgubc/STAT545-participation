...
- what class is  `continent`(a factor or charecter)? - Factor with 5 levels
- How many levels? What are they? - 5 levels, 5 continents
...
f <- gapminder %>% 
  filter(country == 'Canada'|country == 'Austraila'|country == 'China'|country == 'India'|
           country == 'USA' )
...
```{r}
h_gap_dropped <- f %>% droplevels()

h_gap_dropped$country %>% nlevels()
...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_infreq(continent))) + #fct_rev for reverse order plotting
  coord_flip() +
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")
```
...
## fct_reorder(variable1, v2, max) , so max of v2
...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_recode(continent,"OCN"="Oceania", "EU"="Europe"))) +
  coord_flip()+
  theme_bw() 
